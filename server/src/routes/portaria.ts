...
router.get('/current-operation', async (c: Context) => {
    try {
        ...
        if (isMaster) {
            ...
            // Owner: events.organizerId stores organizers.id, so we must fetch the organizer entity ID
            const ownerOrgs = await db.select({ id: organizers.id, userId: organizers.userId }).from(organizers).where(eq(organizers.userId, userId));
            for (const org of ownerOrgs) {
                const orgEvents = await db.select({ id: events.id }).from(events).where(and(eq(events.organizerId, org.id), ne(events.status, 'cancelled'), eq(events.operationStatus, 'open')));
