    ...
    try {
        switch (name) {
            case "create_database": {
                const result = await databases.create(args?.databaseId, args?.name);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "create_collection": {
                const result = await databases.createCollection(args?.databaseId, args?.collectionId, args?.name);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "create_string_attribute": {
                const result = await databases.createStringAttribute(args?.databaseId, args?.collectionId, args?.key, args?.size, args?.required, args?.defaultValue);
                return { content: [{ type: "text", text: "Attribute creation initiated" }] };
            }
            case "create_boolean_attribute": {
                const result = await databases.createBooleanAttribute(args?.databaseId, args?.collectionId, args?.key, args?.required, args?.defaultValue);
                return { content: [{ type: "text", text: "Attribute creation initiated" }] };
            }
            case "create_integer_attribute": {
                const result = await databases.createIntegerAttribute(args?.databaseId, args?.collectionId, args?.key, args?.required, args?.min, args?.max, args?.defaultValue);
                return { content: [{ type: "text", text: "Attribute creation initiated" }] };
            }
            case "list_collections": {
                const result = await databases.listCollections(args?.databaseId);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            default:
